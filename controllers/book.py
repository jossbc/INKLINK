...
from pipelines.book_pipeline import validate_author_exists_pipeline
...
async def search_books_by_title(title: str) -> list[Book]:
    try:
        books = []
        regex = {"$regex": title, "$options": "i"}  
        for doc in coll.find({"title": regex}):
            doc["id"] = str(doc["_id"])
            del doc["_id"]
            books.append(Book(**doc))
        return books
